...
parser.add_argument('--name', default='first_comb', type=str,
                    help='name of experiment')
parser.set_defaults(augment=True)
parser.set_defaults(guassian=False)
...
def main():
    ...
    if turnOn:
        anchor=torch.Tensor(model.needBatch,kernelSizeA,3,32,32).fill_(0)
        for i, (input, target) in enumerate(anchor_loader):
            if i>=model.needBatch:
                break
            anchor[i].copy_(input)
        print("Anchor #",len(anchor))

    for epoch in range(args.start_epoch, args.epochs):
        adjust_learning_rate(optimizer, epoch)

        if turnOn:
            set_anchor(anchor,model)
...
def set_anchor(anchor, model):
    # switch to train mode
    print("Setting anchor")
    model.train()
    model.set_state()  #set flag, renew the variables

    for i in range(len(anchor)):
        input = anchor[i].cuda()
        input_var = torch.autograd.Variable(input)
        model(input_var)
    print("Done Setting anchor.")
